...
class SkiparseMaskPreprocessor(MetaPreprocessor):
    ...
    def preprocess(self, shape, grid_sizes, sequence_preprocessor=None, dtype=torch.bool, device="cuda"):
        ...
        local_token_mask = local_token_mask.squeeze(-1).bool()
        local_group_mask = local_group_mask.squeeze(-1).bool()
        global_token_mask = global_token_mask.squeeze(-1).bool()
...
class OSPNextModel(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        **kwargs,
        ...
        full_shard_seq_lens, token_shard_seq_lens, group_shard_seq_lens = self.sequence_preprocessor.get_shard_seq_lens(
        ...
        local_token_mask, local_group_mask, global_token_mask, global_group_mask = self.mask_preprocessor.preprocess(
        ...
        for idx, block in enumerate(self.blocks):
            ...
            if block.skiparse_block_type == SkiparseBlockType.Full:
                ...
                attn_mask, cross_attn_mask, shard_seq_lens = global_token_mask, local_token_mask, token_shard_seq_lens
            ...
            if use_full_blocks_sp:
                ...
                if idx != len(self.blocks) - 1 and block.is_skiparse2full_block:
                    x = self.sequence_preprocessor.postprocess(
                        x, grid_sizes, shard_seq_lens=full_shard_seq_lens, sp_type=self.main_sp_type
                    )
                    x, sub_grid_sizes = self.sequence_preprocessor.preprocess(
                        x, grid_sizes, sp_type=SequenceParallelType.FullBlocksSP
                    )
